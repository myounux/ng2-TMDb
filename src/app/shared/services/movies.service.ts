...
import {tmdbUrls, tmdbApiKey, language} from '../constantes';
...
export class MoviesService {
	...
	get(id: number): Observable<Movie> {
		return this._http.get(`${tmdbUrls.base}/movie/${id}?api_key=${tmdbApiKey}&language=${language}`)
			.map((response: Response) => response.json())
			.map((data: any) => this.convertToMovie(data));
	}

	getPopular(page = 1): Observable<MultipleMoviesResponse> {
		return this._http.get(`${tmdbUrls.popular}&page=${page}`)
			.map((response: Response) => this.prepareResponse(response));
	}

	getTopRated(page = 1): Observable<MultipleMoviesResponse> {
		return this._http.get(`${tmdbUrls.topRated}&page=${page}`)
			.map((response: Response) => this.prepareResponse(response));
	}

	getUpcoming(page = 1): Observable<MultipleMoviesResponse> {
		return this._http.get(`${tmdbUrls.upcoming}&page=${page}`)
			.map((response: Response) => this.prepareResponse(response));
	}

	getNowPlaying(page = 1): Observable<MultipleMoviesResponse> {
